// subscriber_quic.c — Cliente QUIC que envía "SUBSCRIBE <topic>\n"
// y muestra todo lo que recibe (fiable y ordenado por stream).

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <msquic.h>

#define ALPN_STR "infracom"
#define CHECK_QUIC(st,msg) do{ if(QUIC_FAILED(st)){ fprintf(stderr,"%s:0x%x\n",msg,st); exit(1);} }while(0)

static const QUIC_API_TABLE* MsQuic=NULL;
static HQUIC Registration=NULL, Configuration=NULL;
static HQUIC Conn=NULL, Stream=NULL;
static volatile int done=0;

_Function_class_(QUIC_STREAM_CALLBACK)
static QUIC_STATUS QUIC_API StreamCb(HQUIC S, void* Ctx, QUIC_STREAM_EVENT* E){
    (void)S; (void)Ctx;
    if(E->Type==QUIC_STREAM_EVENT_RECEIVE){
        for(uint32_t i=0;i<E->RECEIVE.BufferCount;i++)
            fwrite(E->RECEIVE.Buffers[i].Buffer,1,E->RECEIVE.Buffers[i].Length,stdout);
        fflush(stdout);
        MsQuic->StreamReceiveComplete(Stream,E->RECEIVE.TotalBufferLength);
    }else if(E->Type==QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE){
        done=1;
    }
    return QUIC_STATUS_SUCCESS;
}

_Function_class_(QUIC_CONNECTION_CALLBACK)
static QUIC_STATUS QUIC_API ConnCb(HQUIC C, void* Ctx, QUIC_CONNECTION_EVENT* E){
    (void)C; (void)Ctx;
    if(E->Type==QUIC_CONNECTION_EVENT_CONNECTED){
        CHECK_QUIC(MsQuic->StreamOpen(Conn,QUIC_STREAM_OPEN_FLAG_NONE,StreamCb,NULL,&Stream),"StreamOpen");
        CHECK_QUIC(MsQuic->StreamStart(Stream,QUIC_STREAM_START_FLAG_IMMEDIATE),"StreamStart");
    } else if(E->Type==QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE){
        done=1;
    }
    return QUIC_STATUS_SUCCESS;
}

int main(int argc,char**argv){
    if(argc!=4){ fprintf(stderr,"Uso: %s <host> <puerto> <topic>\n",argv[0]); return 1; }
    const char* host=argv[1]; uint16_t port=(uint16_t)atoi(argv[2]); const char* topic=argv[3];

    CHECK_QUIC(MsQuicOpenVersion(QUIC_API_VERSION_2,(const void**)&MsQuic),"Open");
    QUIC_REGISTRATION_CONFIG reg={"InfracomQUIC",QUIC_EXECUTION_PROFILE_LOW_LATENCY};
    CHECK_QUIC(MsQuic->RegistrationOpen(&reg,&Registration),"RegOpen");

    QUIC_SETTINGS set={0};
    QUIC_BUFFER alpn={.Length=(uint32_t)strlen(ALPN_STR),.Buffer=(uint8_t*)ALPN_STR};
    CHECK_QUIC(MsQuic->ConfigurationOpen(Registration,&alpn,1,&set,sizeof(set),NULL,&Configuration),"CfgOpen");

    QUIC_CREDENTIAL_CONFIG cred={0};
    cred.Type=QUIC_CREDENTIAL_TYPE_NONE;
    cred.Flags=QUIC_CREDENTIAL_FLAG_CLIENT|QUIC_CREDENTIAL_FLAG_NO_CERTIFICATE_VALIDATION;
    CHECK_QUIC(MsQuic->ConfigurationLoadCredential(Configuration,&cred),"Cred");

    CHECK_QUIC(MsQuic->ConnectionOpen(Registration,ConnCb,NULL,&Conn),"ConnOpen");
    CHECK_QUIC(MsQuic->ConnectionStart(Conn,Configuration,QUIC_ADDRESS_FAMILY_UNSPEC,host,port),"ConnStart");

    // Pequeño delay para asegurar StreamStart
    usleep(300*1000);

    char cmd[256]; snprintf(cmd,sizeof(cmd),"SUBSCRIBE %s\n",topic);
    QUIC_BUFFER qb={.Length=(uint32_t)strlen(cmd),.Buffer=(uint8_t*)cmd};
    MsQuic->StreamSend(Stream,&qb,1,QUIC_SEND_FLAG_NONE,NULL);

    printf("SUBSCRIBER QUIC topic=%s en %s:%u\n",topic,host,port);
    while(!done){ usleep(200*1000); }

    MsQuic->ConnectionShutdown(Conn,QUIC_CONNECTION_SHUTDOWN_FLAG_NONE,0);
    MsQuic->ConnectionClose(Conn); MsQuic->ConfigurationClose(Configuration);
    MsQuic->RegistrationClose(Registration); MsQuicClose(MsQuic);
    return 0;
}
